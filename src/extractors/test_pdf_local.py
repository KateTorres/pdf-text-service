...
Now fully compatible with manual region JSONs measured in true PDF coordinates.
"""
...
def load_regions(region_file):
    ...
    if not os.path.exists(region_file):
        print(f"⚠️  Region file not found: {region_file}")
        return []
...
def extract_text_from_pdf(pdf_path, language="english", start_page=1,
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for i in range(start_page - 1, end_page):
            ...
            if img_count > 0 and char_count < MIN_CHARS_TO_KEEP_PAGE:
                print(f"⏭️  Skipping page {page_num}: image-heavy.")
                continue
            if not words:
                print(f"⏭️  Skipping page {page_num}: no text.")
                continue

            columns = cluster_columns(words, page.width)
            col_count = len(columns)
            print(f"✅ Page {page_num}: detected {col_count} column(s).")
